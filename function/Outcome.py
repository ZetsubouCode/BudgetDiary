...
class Outcome:
    async def add(msg:str):
        ...
        if data[0]=='7':
            ...
            await IncomeController.add(amount, data_date, data[1], "Balancing balance amount")
        await OutcomeController.add(data[0],data[1],data[2],data[3],data_date)
    ...
    async def get_monthly_outcome(data_date):
        ...
        outcome = await OutcomeController.get_monthly_outcome(data_date.replace(day=1),next_month)
        if outcome:
    ...
    async def get_monthly_total(date_input:date):
        ...
        next_month =  next_month - timedelta(days=next_month.day)
        outcome = await OutcomeController.get_monthly_total(date_input,next_month)
